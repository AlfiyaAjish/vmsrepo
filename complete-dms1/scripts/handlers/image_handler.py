...
def build_image(data: ImageBuildRequest, user: TokenData):
    try:
        ...
        tag = build_args.get("tag")
        if tag and not is_valid_docker_tag(tag):
            raise HTTPException(status_code=400, detail=INVALID_REQUEST)
        elif not tag:
            build_args["tag"] = settings.DEFAULT_DOCKER_TAG
...
def build_image_from_github(data: ImageGithubBuildRequest, user: TokenData):
    try:
        ...
        git.Repo.clone_from(data.github_url, temp_dir)
...
def list_images(name: str = None, all: bool = False, filters: Dict[str, Any] = None, user: TokenData = None):
    try:
        kwargs = {}
        if name:
            kwargs["name"] = name
...
def pull_image(repository: str, local_tag: str = None, user: TokenData = None):
    try:
        image = client.images.pull(repository)
