...
from scripts.constants.api_endpoints import Endpoints
from scripts.handlers.vol_handler import create_volume_with_params, remove_volume_with_params
from scripts.models.volume_model import VolumeCreateRequest, VolumeRemoveRequest
...
@volume_router.post(Endpoints.VOLUME_CREATE.replace("/volume", ""))
def create_volume_route(data: VolumeCreateRequest, current_user: TokenData = Depends(get_current_user_from_token)):
    return create_volume_with_params(data, current_user)

@volume_router.delete(Endpoints.VOLUME_DELETE.replace("/volume", ""))
def remove_volume_route(volume_name: str, params: VolumeRemoveRequest, current_user: TokenData = Depends(get_current_user_from_token)):
