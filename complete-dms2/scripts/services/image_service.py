...
def build_image_service(
    ...
    logger.info(f"User '{current_user.username}' building image")
    return build_image(data, current_user)

@image_router.post(Endpoints.IMAGE_BUILD_FROM_GITHUB)
def build_image_from_github_service(
    data: ImageGithubBuildRequest,
    current_user: TokenData = Depends(get_current_user_from_token)
):
...
@image_router.delete(Endpoints.IMAGE_DELETE)
def remove_image_service(
    image_name: str,
    params: ImageRemoveRequest,
    current_user: TokenData = Depends(get_current_user_from_token)
):
