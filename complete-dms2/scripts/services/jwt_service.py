...
from scripts.handlers.jwt_handler import signup_user_handler, login_user_handler, logout_user_handler
from scripts.models.jwt_model import UserSignupRequest, Token, UserLoginRequest, UserLoginResponse, TokenData
from scripts.constants.api_endpoints import Endpoints
from scripts.logging.logger import logger
...
def logout_user(current_user: TokenData = Depends(get_current_user_from_token)):
    logger.info(f"User '{current_user.username}' is logging out.")
    return logout_user_handler(current_user.username)
