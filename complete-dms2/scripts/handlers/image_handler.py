...
def build_image(data: ImageBuildRequest, user: TokenData):
    build_args = data.dict(exclude_unset=True)
    if not build_args.get("path") and not build_args.get("fileobj"):
        raise HTTPException(status_code=400, detail=INVALID_REQUEST)
    if "tag" in build_args and not is_valid_docker_tag(build_args["tag"]):
        raise HTTPException(status_code=400, detail=INVALID_REQUEST)
    build_args.setdefault("tag", settings.DEFAULT_DOCKER_TAG)
    image, _ = client.images.build(**build_args)
    return {
        "message": IMAGE_BUILD_SUCCESS.format(tag=build_args['tag']),
        "id": image.id,
...
def build_image_from_github(data: ImageGithubBuildRequest, user: TokenData):
    temp_dir = tempfile.mkdtemp()
    repo_url = data.github_url
...
def list_images(name: str, all: bool, filters: Dict[str, Any], user: TokenData):
    kwargs = {}
    if name: kwargs["name"] = name
    if all: kwargs["all"] = True
...
def dockerhub_login(username: str, password: str, user: TokenData):
    client.login(username=username, password=password)
    return {"message": AUTH_LOGIN_SUCCESS}

def push_image(local_tag: str, remote_repo: str, user: TokenData):
    image = client.images.get(local_tag)
    image.tag(remote_repo)
    result = client.images.push(remote_repo)
    return {
...
def pull_image(repository: str, local_tag: str, user: TokenData):
    image = client.images.pull(repository)
    if local_tag:
        image.tag(local_tag)
    return {
        "message": IMAGE_PULL_SUCCESS.format(tag=repository),
        "tags": image.tags,
        "retagged_as": local_tag or "Not retagged"
    }

def remove_image(image_name: str, params: ImageRemoveRequest, user: TokenData):
    opts = params.dict(exclude_unset=True)
    client.images.remove(image=image_name, **opts)
    return {
        "message": IMAGE_REMOVE_SUCCESS.format(tag=image_name),
