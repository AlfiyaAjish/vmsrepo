...
def get_rate_limit_handler(user_id: str, current_user: TokenData) -> RateLimitConfig:
    ...
    collection = mongodb.get_collection("rate_limits")
...
def set_rate_limit_handler(user_id: str, limit: int, time_window: int, current_user: TokenData):
    if current_user.role != "Admin":
        raise HTTPException(status_code=403, detail="Admin only")

    collection = mongodb.get_collection("rate_limits")
    if collection.find_one({"user_id": user_id}):
        raise HTTPException(status_code=400, detail="Rate limit already exists")
...
def update_rate_limit_handler(user_id: str, limit: int, time_window: int, current_user: TokenData):
    if current_user.role != "Admin":
        raise HTTPException(status_code=403, detail="Admin only")
