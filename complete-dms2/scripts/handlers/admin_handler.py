...
# mongo = MongoDBConnection()
#
# def list_all_users(user: TokenData = Depends(get_current_user_from_token)):
#     if user.role != "Admin":
#         logger.warning(f"User '{user.username}' attempted to access users list without admin privileges")
#         raise HTTPException(status_code=403, detail="You don't have permission to perform this action.")
#
...
#
#
# def get_user_details(username: str, user: TokenData = Depends(get_current_user_from_token)):
#     if user.role != "Admin":
#         logger.warning(f"User '{user.username}' attempted to access details of user '{username}' without admin privileges")
#         raise HTTPException(status_code=403, detail="You don't have permission to perform this action.")
#
#     try:
#         users_collection = mongo.get_collection(USER_COLLECTION)
...
#
#
# def delete_user(username: str, user: TokenData = Depends(get_current_user_from_token)):
#     if user.role != "Admin":
#         logger.warning(f"User '{user.username}' attempted to delete user '{username}' without admin privileges")
#         raise HTTPException(status_code=403, detail="You don't have permission to perform this action.")
#
...
#
#
# def list_all_containers(user: TokenData = Depends(get_current_user_from_token)):
#     if user.role != "Admin":
#         logger.warning(f"User '{user.username}' attempted to access containers list without admin privileges")
#         raise HTTPException(status_code=403, detail="You don't have permission to perform this action.")
#
...
def list_all_users(user: TokenData):
    if user.role != "Admin":
        raise HTTPException(status_code=403, detail="Admin access required")
    users_collection = mongo.get_collection(USER_COLLECTION)
    return list(users_collection.find({}, {"_id": 0, "password": 0}))

def get_user_details(username: str, user: TokenData):
    if user.role != "Admin":
        raise HTTPException(status_code=403, detail="Admin access required")
    users_collection = mongo.get_collection(USER_COLLECTION)
    user_data = users_collection.find_one({"username": username}, {"_id": 0, "password": 0})
...
def delete_user(username: str, user: TokenData):
    if user.role != "Admin":
        raise HTTPException(status_code=403, detail="Admin access required")
    users_collection = mongo.get_collection(USER_COLLECTION)
    result = users_collection.delete_one({"username": username})
...
def list_all_containers(user: TokenData):
    if user.role != "Admin":
        raise HTTPException(status_code=403, detail="Admin access required")
    containers_collection = mongo.get_collection(CONTAINER_COLLECTION)
    return list(containers_collection.find({}, {"_id": 0}))
