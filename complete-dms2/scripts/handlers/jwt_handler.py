...
from scripts.logging.logger import logger
from passlib.context import CryptContext
from scripts.constants.app_constants import AUTH_USER_EXISTS, AUTH_LOGIN_FAILURE, AUTH_SIGNUP_SUCCESS, AUTH_SIGNUP_FAILURE
...
def login_user_handler(user_login: UserLoginRequest) -> UserLoginResponse:
    users_collection = mongodb.get_collection("users")
    user_record = users_collection.find_one({"username": user_login.username})

    if not user_record or not pwd_context.verify(user_login.password, user_record["password"]):
        logger.warning(f"Login failed for user '{user_login.username}'")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=AUTH_LOGIN_FAILURE
        )

    logger.info(f"User '{user_login.username}' authenticated successfully")
    access_token = create_user_token(user_login.username, user_record["role"])
